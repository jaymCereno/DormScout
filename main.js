...
function searchDorms() {
    // Sync custom dropdowns to hidden selects (for safety)
        ...
        if (filterType && selected) {
            ...
            switch(filterType) {
                case 'location':
                    ...
                    break;
                case 'distance':
                    document.getElementById('distanceFilter').value = value;
                    break;
                case 'price':
...
function applyFiltersFromURL() {
    ...
    // Also update custom dropdowns to reflect the selected value
    const filterMap = {
        location: location,
        distance: distance,
        price: price
    };
    Object.keys(filterMap).forEach(function(filterType) {
        const value = filterMap[filterType];
        ...
        if (customSelect) {
            ...
            items.forEach(function(item) {
                if ((value === null || value === '' || value === undefined) && item.getAttribute('data-value') === '') {
                    selected.textContent = item.textContent;
                    selected.setAttribute('data-value', '');
                    items.forEach(i => i.classList.remove('same-as-selected'));
                    item.classList.add('same-as-selected');
                    found = true;
                } else if (item.getAttribute('data-value') === value) {
                    selected.textContent = item.textContent;
            ...
            // fallback: if not found, set to first
            if (!found && items.length > 0) {
                selected.textContent = items[0].textContent;
                selected.setAttribute('data-value', items[0].getAttribute('data-value'));
                items.forEach(i => i.classList.remove('same-as-selected'));
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (listing) {
        ...
        dorms.forEach((dorm) => {
            dorm.style.opacity = '1';
            dorm.style.transform = 'none';
            dorm.style.transition = 'none';
        });
    }
...
function initSelectDormButtons() {
    ...
    selectButtons.forEach(button => {
        button.addEventListener('click', function(e) {
            ...
            // Redirect to profile page instantly
